...
local function http_get(url)
    if type(request)=="function" then
        ...
        if ok then
            ...
            if type(res)=="table" then return res.Body or res.body or res.ResponseBody end
        end
    end
...
local cfg_defaults = {
    ...
    team_check=true, training=true,
    ap_indicator=true, ap_legacy=true, ap_cal=false, ap_los=true,
    ap_lead=50, ap_fallback=320, ap_siege_delay=500,
    ap_phx=true, ap_phx_lead=60, ap_phx_radius=30,
    ap_draw={castigate=750, phoenix=800, siege=1100, monarch=1850, basegun=750},
    ap_smax={castigate=0.30, phoenix=0.30, siege=0.30, monarch=0.30, basegun=0.30},
    theme='purple',
    auto_save=true,
}
...
local st = {
    ...
    aura_t=0, att=nil, att_t=0, hb_on=true,
}
...
local on_cassette, on_window, on_flash, on_parry, on_melee, try_melee, is_own, gun_ray_aims_at_me
...
local function load_offsets()
    local ok,txt=pcall(function() return http_get("https://offsets.imtheo.lol/offsets.hpp") end)
...
local team_cache={}; local team_cache_t=0
local function match_team_id(p)
    if not p then return nil end
    local now=oc()
    if now-team_cache_t>0.25 then team_cache={}; team_cache_t=now end
    if team_cache[p]~=nil then return team_cache[p]~=false and team_cache[p] or nil end
    local ro=try(function() return rs:FindFirstChild("ReadOnly") end)
    local match=ro and try(function() return ro:FindFirstChild("Match") end)
    local match_plrs=match and try(function() return match:FindFirstChild("Players") end)
    local uid=try(function() return p.UserId end)
    local pdata=match_plrs and uid and try(function() return match_plrs:FindFirstChild(tostring(uid)) end)
    local team_id=pdata and try(function() return pdata:GetAttribute("team_id") end)
    team_id=tonumber(team_id)
    team_cache[p]=team_id or false
    return team_id
end

local function is_enemy(p)
    if not cfg.team_check then return true end
    if not lp or not p or is_self(p) then return false end
    local my_team=match_team_id(lp)
    local their_team=match_team_id(p)
    if my_team==nil or their_team==nil then return false end
    return my_team~=their_team
end

local function player_from_ref(ref)
    if not ref then return nil end
    for _,p in ipairs(try(function() return plrs:GetPlayers() end) or {}) do
        if ref==p then return p end
        local ch=try(function() return p.Character end)
        if ch and ref==ch then return p end
        local rn=try(function() return ref.Name end)
        if rn and rn==p.Name then return p end
    end
    return nil
end

local function is_hostile_ref(ref)
    if not cfg.team_check then return true end
    local p=player_from_ref(ref)
    if p then return is_enemy(p) end
    return cfg.training
end
...
local function near_ent(pos)
    ...
    for _,obj in ipairs(try(function() return workspace:GetChildren() end) or {}) do
        ...
        if not try(function() return obj:IsA("Model") end) then continue end
    ...
    if best and not is_hostile_ref(best) then return nil end
    return best
end
    ...
    if inc then
        ...
        if ef then
            for _,e in ipairs(try(function() return ef:GetChildren() end) or {}) do
                ...
                local ep=player_from_ref(e)
                if ep and not is_enemy(ep) then continue end
        ...
        for _,obj in ipairs(try(function() return workspace:GetChildren() end) or {}) do
            ...
            if try(function() return obj.Name end)==lp.Name then continue end
            local op=player_from_ref(obj)
            if op and not is_enemy(op) then continue end
            local hum=try(function() return obj:FindFirstChildOfClass("Humanoid") end); if not hum then continue end
...
local function cfg_load()
    ...
    if ok and data and data~="" then
        cfg_apply(data); cfg.ap_los=true; log("[cfg] loaded")
...
task.spawn(function()
    while loops_active do
        ...
        for _,entry in ipairs(parry_queue) do
            ...
            elseif cfg.team_check and (not entry.att or not is_hostile_ref(entry.att)) then
                entry.done=true
                dlog("[pq] skip: teammate or unresolved shooter")
            elseif not entry.att or not gun_ray_aims_at_me or not gun_ray_aims_at_me(entry.att) then
                entry.done=true
                dlog("[pq] skip: shooter head ray is not hitting local character")
...
local indicator_shots={}
local indicator_conn=nil
local indicator_watch_t=0
local indicator_poll_t=0
local indicator_recent_until=0
local indicator_siege_until=0
local indicator_last_press=0
local indicator_rocket_track={}
local indicator_phx_arms={}
local AP_FLAT=Vector3.new(1,0,1)
local AP_ARC_SETTLE=0.06
local AP_ARC_MIN_FRAC=0.40
local AP_PROJ_MAX=450
local AP_ARC_ALPHA={castigate=0.90,phoenix=0.90,siege=0.92,monarch=0.94,basegun=0.90,__default=0.90}

local function ap_head_pos(model)
    if not model then return nil end
    local p=player_from_ref(model)
    local src=(p and try(function() return p.Character end)) or model
    local h=try(function() return src:FindFirstChild("Head") end)
    if h then return try(function() return h.Position end) end
    local r=try(function() return src:FindFirstChild("HumanoidRootPart") end)
        or try(function() return src.PrimaryPart end)
        or try(function() return src:FindFirstChildWhichIsA("BasePart") end)
    return r and try(function() return r.Position end) or nil
end

local function ap_candidates()
    local out,seen={},{}
    local my=get_char()
    local ents=try(function() return workspace:FindFirstChild(cfg.ents) end)
    if ents then
        for _,m in ipairs(try(function() return ents:GetChildren() end) or {}) do
            if m~=my and not seen[m] and try(function() return m:IsA("Model") end) then
                local p=player_from_ref(m)
                if p or cfg.training then seen[m]=true; out[#out+1]=m end
            end
        end
    end
    for _,p in ipairs(try(function() return plrs:GetPlayers() end) or {}) do
        if not is_self(p) then
            local ch=try(function() return p.Character end)
            if ch and ch~=my and not seen[ch] then seen[ch]=true; out[#out+1]=ch end
        end
    end
    return out
end

local function ap_game_angle(pos)
    local cam=workspace.CurrentCamera
    if not cam or not pos then return nil end
    local cf=try(function() return cam.CFrame end); if not cf then return nil end
    local look=cf.LookVector*AP_FLAT
    if look.Magnitude<1e-4 then return nil end
    look=look.Unit
    local v=(cf.Position*AP_FLAT)-(pos*AP_FLAT)
    if v.Magnitude<1e-4 then return nil end
    local u=v.Unit
    local dot=look:Dot(u)
    local cross=look.X*u.Z-look.Z*u.X
    return dg(math.atan2(cross,dot))
end

local function ap_ang_diff(a,b)
    return abs(((a-b+180)%360)-180)
end

local function ap_find_shooter(ind_ang)
    if ind_ang==nil then return nil,false end
    local best,bd=nil,math.huge
    local friendly_d=math.huge
    for _,m in ipairs(ap_candidates()) do
        local hp=ap_head_pos(m)
        local a=hp and ap_game_angle(hp)
        if a then
            local d=ap_ang_diff(a,ind_ang)
            local p=player_from_ref(m)
            if cfg.team_check and p and not is_enemy(p) and d<friendly_d then friendly_d=d end
            if d<bd then best=m; bd=d end
        end
    end
    if not best or bd>14 then return nil,false end
    if cfg.team_check then
        local p=player_from_ref(best)
        if p and not is_enemy(p) then return best,true end
        if friendly_d<=bd+2.5 then return nil,true end
    end
    return best,false
end

local AP_GUNS={castigate=true,phoenix=true,siege=true,monarch=true,basegun=true}
local function ap_gun_of(model)
    if not model then return nil end
    for _,ch in ipairs(try(function() return model:GetChildren() end) or {}) do
        local id=try(function() return ch:GetAttribute("item_id") end)
        id=type(id)=="string" and string.lower(id) or nil
        if id and AP_GUNS[id] then return id end
    end
    for _,ch in ipairs(try(function() return model:GetChildren() end) or {}) do
        local nm=string.lower(try(function() return ch.Name end) or "")
        if AP_GUNS[nm] and try(function() return ch:IsA("Model") end) then return nm end
    end
    return nil
end

local function ap_draw_for(gun)
    local tbl=type(cfg.ap_draw)=="table" and cfg.ap_draw or {}
    return tonumber(tbl[gun]) or tonumber(tbl.castigate) or 750
end
local function ap_smax_for(gun)
    local tbl=type(cfg.ap_smax)=="table" and cfg.ap_smax or {}
    return tonumber(tbl[gun]) or 0.30
end
local function ap_arc_alpha(gun)
    return AP_ARC_ALPHA[gun] or AP_ARC_ALPHA.__default
end

local function ap_s_from_rot(r)
    if r>180 then r=360-r end
    if r<0 then r=0 end
    return 1-math.cos(math.rad(r)/2)
end

local function ap_read_arc(ind)
    local pr=try(function() return ind:FindFirstChild("ParryRange") end)
    local lc=pr and try(function() return pr:FindFirstChild("LeftClip") end)
    local left=lc and try(function() return lc:FindFirstChild("Left") end)
    local grad=left and try(function() return left:FindFirstChild("UIGradient") end)
    if not grad then return nil end
    local r=try(function() return grad.Rotation end)
    local prr=try(function() return pr.Rotation end)
    if type(r)~="number" or type(prr)~="number" then return nil end
...
local function ap_has_los(shooter)
    return gun_ray_aims_at_me(shooter)
end

local function ap_face_pos(pos)
    if not cfg.gp_aim or not pos then return end
    local cam=workspace.CurrentCamera; if not cam then return end
    local cf=try(function() return cam.CFrame end); if not cf then return end
    local flat=(pos-cf.Position)*AP_FLAT
    if flat.Magnitude<1e-3 then return end
    local y=cf.LookVector.Y
    local look=Vector3.new(flat.Unit.X,y,flat.Unit.Z)
    if look.Magnitude<1e-3 then return end
    local ok=pcall(function() cam.CFrame=CFrame.lookAt(cf.Position,cf.Position+look.Unit) end)
    if ok then return end
    local vp=try(function() return cam.ViewportSize end)
    local scr=try(function() return WorldToScreen(pos) end)
    if vp and scr and type(scr)~="boolean" and type(mousemoverel)=="function" then
        local dx=(scr.X-vp.X/2)*0.7; local dy=(scr.Y-vp.Y/2)*0.7
        local mag=sq(dx*dx+dy*dy)
        if mag>30 then local s=30/mag; dx=dx*s; dy=dy*s end
        pcall(mousemoverel,0,fl(dx),fl(dy))
    end
end

local function ap_add_indicator(inst,late)
    if not cfg.ap_indicator or not inst or indicator_shots[inst] then return end
    indicator_shots[inst]={
        t0=oc(),maxS=0,curS=nil,lastAngle=nil,
        shooter=nil,gun=nil,handled=false,late=late,
        adjusted=not late,blocked=false,warned=false,phx_armed=false,legacy_cancelled=false,
    }
    indicator_recent_until=ms()+2600
end

local function ap_watch_gui()
    indicator_watch_t=oc()
    if indicator_conn then pcall(function() indicator_conn:Disconnect() end); indicator_conn=nil end
    local pg=lp and try(function() return lp:FindFirstChild("PlayerGui") end)
    if not pg then return end
    local ok,conn=pcall(function()
        return pg.DescendantAdded:Connect(function(inst)
            if try(function() return inst.Name end)=="ShooterIndicator" then ap_add_indicator(inst,false) end
        end)
    end)
    if ok then indicator_conn=conn end
end

local function ap_poll_indicators()
    local now=oc()
    if now-indicator_poll_t<0.10 then return end
    indicator_poll_t=now
    local pg=lp and try(function() return lp:FindFirstChild("PlayerGui") end)
    if not pg then return end
    local root=try(function() return pg:FindFirstChild("GameplayUI",true) end) or pg
    for _,d in ipairs(try(function() return root:GetDescendants() end) or {}) do
        if try(function() return d.Name end)=="ShooterIndicator" then ap_add_indicator(d,true) end
    end
end

local function ap_calibrate(st,lifetime)
    if not cfg.ap_cal then return end
    local gun=st.gun or "castigate"
    if lifetime>0.20 and lifetime<2.40 then
        cfg.ap_draw[gun]=cl((cfg.ap_draw[gun] or 750)+0.30*(lifetime*1000-(cfg.ap_draw[gun] or 750)),200,2400)
    end
    if st.maxS>0.08 and st.maxS<0.80 then
        cfg.ap_smax[gun]=cl((cfg.ap_smax[gun] or 0.30)+0.30*(st.maxS-(cfg.ap_smax[gun] or 0.30)),0.08,0.80)
    end
end

local function ap_arm_phoenix(shooter,hostile)
    if not shooter then return end
    indicator_phx_arms[shooter]={hostile=hostile,expires=oc()+5}
end

local function ap_effect_pos(inst)
    if not inst then return nil end
    if try(function() return inst:IsA("BasePart") end) then return try(function() return inst.Position end) end
    local pp=try(function() return inst.PrimaryPart end)
    if pp then return try(function() return pp.Position end) end
    local bp=try(function() return inst:FindFirstChildWhichIsA("BasePart",true) end)
    if bp then return try(function() return bp.Position end) end
    return try(function() return inst:GetPivot().Position end)
end

local function ap_assign_rocket(pos)
    local now=oc()
    local best,bd,count=nil,math.huge,0
    for shooter,meta in pairs(indicator_phx_arms) do
        if now>meta.expires then indicator_phx_arms[shooter]=nil
        else
            count=count+1
            local hp=ap_head_pos(shooter)
            local d=hp and dsq(pos,hp) or math.huge
            if d<bd then best=shooter; bd=d end
        end
    end
    if best and (bd<=10000 or (not cfg.team_check and count==1)) then
        local meta=indicator_phx_arms[best]
        return meta and meta.hostile or false,best
    end
    return not cfg.team_check,nil
end

local function ap_is_rocket(name)
    return string.find(string.lower(name or ""),"rocket",1,true)~=nil
end

local function ap_watch_rockets(now,dt)
    if not cfg.ap_phx then return false end
    local eff=try(function() return workspace:FindFirstChild("Effects") end)
    local me=get_pos()
    if not eff or not me then return false end
    local present={}
    local best_meta,best_pos,best_dist,best_tti=nil,nil,nil,math.huge
    for _,inst in ipairs(try(function() return eff:GetChildren() end) or {}) do
        local nm=try(function() return inst.Name end) or ""
        if ap_is_rocket(nm) then
            local pos=ap_effect_pos(inst)
            if pos then
                present[inst]=true
                local dist=sq(dsq(pos,me))
                local meta=indicator_rocket_track[inst]
                if not meta then
                    local hostile,shooter=ap_assign_rocket(pos)
                    meta={dist=dist,hostile=hostile,shooter=shooter,handled=false}
                    indicator_rocket_track[inst]=meta
                else
                    local closing=(meta.dist-dist)/mx(dt or 0,0.0001)
                    meta.dist=dist
                    if meta.hostile and not meta.handled and closing>5 and dist<AP_PROJ_MAX then
                        local tti=dist/closing
                        if not best_dist or dist<best_dist then
                            best_meta,best_pos,best_dist,best_tti=meta,pos,dist,tti
                        end
                    end
                end
            end
    ...
    for inst in pairs(indicator_rocket_track) do
        if not present[inst] then indicator_rocket_track[inst]=nil end
    end
    if best_meta and (best_dist<=(cfg.ap_phx_radius or 30) or best_tti<=((cfg.ap_phx_lead or 60)/1000)) then
        best_meta.handled=true
        ap_face_pos(best_pos)
        task.spawn(function()
            if gun_press() then
                indicator_last_press=ms(); last_gp_press=ms(); dlog("[ap arc] F -> phoenix impact")
            end
        end)
        return true
    end
    return best_meta~=nil
end

local function ap_should_fire(st,now,gun)
    local elapsed=(now-st.t0)*1000
    if elapsed<30 then return false,elapsed end
    if st.noArc then return elapsed>=(cfg.ap_fallback or 320),elapsed end
    local draw=ap_draw_for(gun)
    if elapsed>=draw-(cfg.ap_lead or 50) then return true,elapsed end
    if st.curS and st.curS>0.001 and elapsed>=AP_ARC_MIN_FRAC*draw then
        if st.curS/ap_smax_for(gun)>=ap_arc_alpha(gun) then return true,elapsed end
    end
    return false,elapsed
end

local function ap_cancel_legacy(att)
    local now=ms()
    local ap=player_from_ref(att)
    for _,e in ipairs(parry_queue) do
        local same=(att and e.att==att) or (ap and player_from_ref(e.att)==ap)
        if not e.done and (same or abs(e.fire_at-now)<500) then e.done=true end
    end
end

local function indicator_step(dt)
    if not loops_active or not cfg.gp or not cfg.ap_indicator then return end
    if not indicator_conn and oc()-indicator_watch_t>2 then ap_watch_gui() end
    ap_poll_indicators()
    local now=oc()
    local rocket_incoming=ap_watch_rockets(now,dt)
    local fire_st,fire_gun,fire_elapsed=nil,nil,-1

    for inst,st2 in pairs(indicator_shots) do
        local alive=try(function() return inst.Parent end)
        if not alive then
            ap_calibrate(st2,now-st2.t0)
            indicator_shots[inst]=nil
        else
            indicator_recent_until=ms()+250
            if now-st2.t0>=AP_ARC_SETTLE then
                local s,ang=ap_read_arc(inst)
                if s~=nil then
                    st2.curS=s; if s>st2.maxS then st2.maxS=s end
                    if ang~=nil then st2.lastAngle=ang end
                    st2.noArc=nil
                elseif st2.maxS<=0 and now-st2.t0>0.15 then
                    st2.noArc=true
                end
            end

            if st2.lastAngle and (not st2.shooter or st2.blocked) then
                local shooter,blocked=ap_find_shooter(st2.lastAngle)
                if shooter then st2.shooter=shooter end
                st2.blocked=blocked
            end
            if st2.shooter and not st2.gun then st2.gun=ap_gun_of(st2.shooter) or false end
            local gun=(type(st2.gun)=="string" and st2.gun) or "castigate"

            if st2.late and not st2.adjusted and st2.curS then
                local frac=cl(st2.curS/ap_smax_for(gun),0,0.98)
                st2.t0=now-(frac*ap_draw_for(gun)/1000)
                st2.adjusted=true
            end

            local hostile=true
            if cfg.team_check then
                if not st2.shooter or st2.blocked then hostile=false
                else hostile=is_hostile_ref(st2.shooter) end
            end

            if hostile and st2.shooter and not st2.legacy_cancelled then
                ap_cancel_legacy(st2.shooter)
                st2.legacy_cancelled=true
            end

            if hostile and not st2.warned and cfg.warn and st2.shooter then
                st2.warned=true
                warn_til=tick()+mx(0.45,ap_draw_for(gun)/1000)
                warn_blink_t=tick()
                rn("incoming "..gun,"Redline",1)
            end

            if not st2.handled then
                local due,elapsed=ap_should_fire(st2,now,gun)
                if due then
                    if not hostile then
                        if st2.shooter and st2.blocked then st2.handled=true; dlog("[ap arc] teammate ignored") end
                    elseif not ap_has_los(st2.shooter) then
                        dlog("[ap arc] hold: shooter head ray is not hitting local character")
                    elseif gun=="phoenix" and cfg.ap_phx then
                        if not st2.phx_armed then
                            ap_arm_phoenix(st2.shooter,true)
                            st2.phx_armed=true
                        end
                        st2.handled=true
                    elseif elapsed>fire_elapsed then
                        fire_st,fire_gun,fire_elapsed=st2,gun,elapsed
                    end
                end
            end
        end
    ...
    if fire_st and not rocket_incoming and ap_has_los(fire_st.shooter) then
        fire_st.handled=true
        ap_cancel_legacy(fire_st.shooter)
        local hp=ap_head_pos(fire_st.shooter)
        if hp then ap_face_pos(hp) end
        if fire_gun=="siege" then indicator_siege_until=ms()+2500 end
        local fire_shooter=fire_st.shooter
        task.spawn(function()
            if not gun_ray_aims_at_me(fire_shooter) then return end
            if gun_press() then
                indicator_last_press=ms(); last_gp_press=ms()
                dlog("[ap arc] F -> "..tostring(fire_gun))
                if fire_gun=="siege" and cfg.s2 then
                    task.wait((cfg.ap_siege_delay or 500)/1000)
                    if loops_active and cfg.gp and cfg.s2 and gun_ray_aims_at_me(fire_shooter) then
                        gun_press(true); dlog("[ap arc] F -> siege (2nd)")
                    end
                end
            end
        end)
...
on_window=function(gun_guess,epos,src,certain)
    if not cfg.gp or not cfg.ap_legacy then return end
    if cfg.ap_indicator and ms()<indicator_recent_until then dlog("[gp] indicator owns shot"); return end
    local me0=get_pos()
    ...
    if epos then
        ...
        if cand then
            ...
            att=cand; st.att=att; st.att_t=ms()
        end
    end
    if not att then dlog("[gp] no shooter attribution, skip"); return end
    if cfg.team_check and not is_hostile_ref(att) then dlog("[gp] teammate shooter ignored"); return end
    if not gun_ray_aims_at_me(att) then dlog("[gp] shooter head ray missed local character"); return end
    local gun=gun_guess
...
on_flash=function(gun,fpos)
    ...
    if gun=="siege" and cfg.s2 and cfg.gp and cfg.ap_legacy
        and (not cfg.team_check or firer~=nil)
        and not (cfg.ap_indicator and ms()<indicator_siege_until) then
        local now_s2=ms()
        if now_s2-siege_s2_t>((cfg.s2_w2f or 1000)+300) then  -- one 2nd-parry per whole siege cycle, not per flash
            ...
            task.spawn(function()
                ...
                if cfg.gp and cfg.s2 and firer and gun_ray_aims_at_me(firer) then
                    gun_press(true); dlog("[gp] F -> siege (2nd)")
                end
            end)
        end
    end
...
on_melee=function(ev,src)
    ...
    if not(ok and kpos and (kpos.X~=0 or kpos.Y~=0 or kpos.Z~=0)) then
        if cfg.team_check then dlog("[mp] zero-pos swing ignored: shooter cannot be team-verified"); return end
    ...
    if cfg.team_check and (not att or not is_hostile_ref(att)) then dlog("[mp] teammate/unattributed swing ignored"); return end
...
local function scan_pgui()
    if not cfg.gp or not cfg.ap_legacy then return end
    if cfg.ap_indicator and ms()<indicator_recent_until then return end
    ...
    for eff_nm,gun in pairs(pgui_map) do
            ...
            if shot and shot.claimed then
                if shot.gun~=gun then
                    ...
                    elseif fire_at>ms()+40 then  -- wrong press already fired, but real gun fires later -> queue it
                        shot.entry=enqueue_parry(fire_at-ms(),st.att,gun,nil); shot.gun=gun
        ...
        if elapsed>=delay then
            ...
            local att=st.att
            local fresh=att and (ms()-(st.att_t or 0))<=2500
            local hostile=fresh and ((not cfg.team_check) or is_hostile_ref(att))
            if not hostile or not gun_ray_aims_at_me(att) then
                dlog("[gp] pgui skip: no hostile shooter head ray on local character")
                continue
            end
            if not shot then new_shot() end
            maybe_reset_shot()
            ...
            local _gun,_att=gun,att
            task.spawn(function()
                if not gun_ray_aims_at_me(_att) then return end
                dlog("[gp] F -> ".._gun.." (pgui)")
    ...
    while loops_active do
        if lp then
            ...
            for _,p in ipairs(all) do
                set[p]=true
                if not is_self(p) and is_enemy(p) then mk_esp(p)
                elseif esp_obj[p] then rm_esp(p) end
            end
            for p in next,esp_obj do
                if not set[p] or is_self(p) or not is_enemy(p) then rm_esp(p) end
            end
        end
        task.wait(0.5)
    end
...
task.spawn(function()
    local last_step=oc()
    while loops_active do
        local now=oc(); local dt=now-last_step; last_step=now
        indicator_step(dt)
        scan_effects()
        scan_pgui()
        task.wait(1/get_fps())
...
_G.rl_cleanup=function()
    loops_active=false
    if indicator_conn then pcall(function() indicator_conn:Disconnect() end); indicator_conn=nil end
    indicator_shots={}; indicator_rocket_track={}; indicator_phx_arms={}
    rm_hud(); rm_fov()
...
task.spawn(function()
    ...
    local src=try(function() return http_get("https://raw.githubusercontent.com/shystemcito/ForMatcha-Testing/refs/heads/main/Libs/Loader.luau") end)
    ...
    -- indicator arc parry
    pcall(function()
        local c=Window.AddCategory("Arc Parry")
        Window.AddSection(c,"Shooter Indicator")
        tip(Window.AddToggle(c,"Indicator timing",cfg.ap_indicator,function(s)
            cfg.ap_indicator=s
            if s then ap_watch_gui() else indicator_shots={} end
            mark_chg()
        end),"uses ShooterIndicator arc progress and per-gun draw timing; this is the primary gun parry path")
        tip(Window.AddToggle(c,"Legacy fallback",cfg.ap_legacy,function(s) cfg.ap_legacy=s; mark_chg() end),"keeps the old effects and glare detector available when no ShooterIndicator is active")
        tip(Window.AddToggle(c,"Auto calibrate",cfg.ap_cal,function(s) cfg.ap_cal=s; mark_chg() end),"refines each gun's draw time and terminal arc from observed indicators")
        Window.AddSection(c,"Head Aim Raycast: Required")
        Window.AddSlider(c,"Lead ms",0,400,cfg.ap_lead,function(v) cfg.ap_lead=fl(v); mark_chg() end)
        Window.AddSlider(c,"Fallback ms",50,1000,cfg.ap_fallback,function(v) cfg.ap_fallback=fl(v); mark_chg() end)
        Window.AddSection(c,"Per-Gun Draw")
        Window.AddSlider(c,"Castigate draw ms",200,2400,cfg.ap_draw.castigate,function(v) cfg.ap_draw.castigate=fl(v); mark_chg() end)
        Window.AddSlider(c,"Monarch draw ms",500,2400,cfg.ap_draw.monarch,function(v) cfg.ap_draw.monarch=fl(v); mark_chg() end)
        Window.AddSlider(c,"Siege draw ms",300,2000,cfg.ap_draw.siege,function(v) cfg.ap_draw.siege=fl(v); mark_chg() end)
        Window.AddSlider(c,"Phoenix draw ms",300,2000,cfg.ap_draw.phoenix,function(v) cfg.ap_draw.phoenix=fl(v); mark_chg() end)
    end)

    pcall(function()
        local c=Window.AddCategory("Projectile Parry")
        Window.AddSection(c,"Phoenix")
        tip(Window.AddToggle(c,"Impact parry",cfg.ap_phx,function(s) cfg.ap_phx=s; mark_chg() end),"suppresses the Phoenix draw-end press and parries the incoming Rocket near impact")
        Window.AddSlider(c,"Impact lead ms",0,300,cfg.ap_phx_lead,function(v) cfg.ap_phx_lead=fl(v); mark_chg() end)
        Window.AddSlider(c,"Impact radius",5,80,cfg.ap_phx_radius,function(v) cfg.ap_phx_radius=fl(v); mark_chg() end)
        Window.AddSection(c,"Siege")
        Window.AddSlider(c,"Indicator 2nd delay ms",50,1500,cfg.ap_siege_delay,function(v) cfg.ap_siege_delay=fl(v); mark_chg() end)
    end)
    ...
    pcall(function()
        ...
        Window.AddSection(c,"Targeting")
        tip(Window.AddToggle(c,"Team check",cfg.team_check,function(v) cfg.team_check=v; tgt_cache_t=0; team_cache_t=0; mark_chg() end),"uses match team_id and ignores teammates for auto parry, melee parry, aim, aura, hitbox, and ESP")
