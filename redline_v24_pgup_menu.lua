-- redline.lua | koji_xyz | v24-teamcheck-aimray-pgup
...
local cfg_defaults = {
    ...
    gp=true, gp_aim=true, gp_dist=1000, gp_ray=true,
...
local shot=nil
local function new_shot() shot={t=nil, t0=nil, claimed=false, certain=false, gun=nil, att=nil, entry=nil} end
new_shot()
...
local function match_team_id(p)
    ...
    return tonumber(team_id)
end

local function is_enemy(p)
    if not cfg.team_check or not lp or not p then return true end
    local my_team=match_team_id(lp)
    local their_team=match_team_id(p)
    if my_team==nil or their_team==nil then return true end
    return my_team~=their_team
end

local function ent_model(e)
    if not e then return nil end
    local ok,ch=pcall(function() return e.Character end)
    if ok and ch then return ch end
    return e
end

local function ent_player(e)
    if not e then return nil end
    local ok,is_p=pcall(function() return e:IsA("Player") end)
    if ok and is_p then return e end
    local model=ent_model(e)
    return model and try(function() return plrs:GetPlayerFromCharacter(model) end) or nil
end

local function head_pos(e)
    if not e then return nil end
    local src=ent_model(e)
...
local function ray_hits_local(att)
    if not cfg.gp_ray then return true end
    local target=get_char()
    local src=ent_model(att)
    if not target or not src or src==target then return false end

    local hd=try(function() return src:FindFirstChild("Head") end)
        or try(function() return src:FindFirstChild("HumanoidRootPart") end)
    local cf=hd and try(function() return hd.CFrame end)
    if not cf then return false end

    local origin=cf.Position
    local look=cf.LookVector
    local maxd=math.max(1,cfg.gp_dist or 1000)
    ...
    local ok,result=pcall(function()
        ...
        return workspace:Raycast(origin,look*maxd,params)
    end)

    if ok then
        if not result then return false end
        local hit=try(function() return result.Instance end)
        return hit and try(function() return hit:IsDescendantOf(target) end)==true
    end

    -- Fallback for executors without workspace:Raycast.
    for _,part in ipairs(try(function() return target:GetDescendants() end) or {}) do
        local ok_part,is_part=pcall(function() return part:IsA("BasePart") end)
        if not(ok_part and is_part) then continue end
        local p=try(function() return part.Position end)
        local sz=try(function() return part.Size end)
        if not p or not sz then continue end
        local rel=p-origin
        local along=rel.X*look.X+rel.Y*look.Y+rel.Z*look.Z
        if along<0 or along>maxd then continue end
        local closest=origin+look*along
        local radius=sq(sz.X*sz.X+sz.Y*sz.Y+sz.Z*sz.Z)*0.5
        if dsq(closest,p)<=radius*radius then return true end
    end
    return false
end

local function gun_attacker_ok(att)
    if not att then return false,"no attacker" end
    local p=ent_player(att)
    if p then
        if is_self(p) then return false,"self" end
        if not is_enemy(p) then return false,"teammate" end
    end
    if not ray_hits_local(att) then return false,"not aiming at local character" end
    return true
end

local function find_aiming_enemy()
    local my=get_pos()
    if not my then return nil end
    local best,bd=nil,math.huge
    local maxd=(cfg.gp_dist or 1000)^2
    for _,p in ipairs(try(function() return plrs:GetPlayers() end) or {}) do
        ...
        local d=dsq(my,hp)
        if d<=maxd and d<bd and ray_hits_local(p) then
            best=p; bd=d
        end
    end
    return best
...
on_window=function(gun_guess,epos,src,certain)
    ...
    local att_ok,att_reason=gun_attacker_ok(att)
    if not att_ok then
        dlog("[gp] ignore: "..tostring(att_reason))
        return
    end
    st.att=att
    ...
    shot.claimed=true; shot.gun=gun; shot.att=att; shot.t=ms(); shot.t0=ms()
...
on_flash=function(gun,fpos)
    ...
    local siege_claimed=shot and shot.claimed and shot.gun=="siege" and shot.att~=nil
    if siege_claimed and firer then
        local a=ent_player(shot.att) or ent_model(shot.att)
        local b=ent_player(firer) or ent_model(firer)
        siege_claimed=(a==b)
    end
    ...
    if gun=="siege" and cfg.s2 and cfg.gp and siege_claimed then
        local now_s2=ms()
...
local function scan_pgui()
    ...
    for eff_nm,gun in pairs(pgui_map) do
        ...
        if elapsed>=delay then
            ...
            local pg_att=find_aiming_enemy()
            if not pg_att then
                dlog("[gp] pgui ignore: no hostile head ray hit local character")
                continue
            end
            shot.claimed=true; shot.gun=gun; shot.att=pg_att; shot.certain=true; shot.t=ms(); shot.t0=ms()
            local _gun=gun
            task.spawn(function()
                dlog("[gp] F -> ".._gun.." (pgui)")
...
task.spawn(function()
    ...
    pcall(function()
        ...
        tip(Window.AddToggle(c,"Aim ray check",cfg.gp_ray,function(s) cfg.gp_ray=s; mark_chg() end),"only parry when a hostile shooter's head ray hits your character")
    ...
    pcall(function()
        ...
        tip(Window.AddToggle(c,"Team check",cfg.team_check,function(v) cfg.team_check=v; tgt_cache_t=0; mark_chg() end),"uses the match team_id attribute and ignores teammates for aim, aura, hitbox, and ESP")
    ...
    pcall(function()
        ...
        Window.AddValueLabel(c,"Menu key","Page Up")
    ...
    apply_theme(cfg.theme)
    pcall(function() Window.SetVisible(false) end)  -- start minimized

    -- menu toggle key (Page Up)
    task.spawn(function()
        local last=false
        while loops_active do
            local active=false; pcall(function() active=isrbxactive() end)
            if active then
                local vk=0x21  -- VK_PRIOR / Page Up
    ...
    pcall(function() UiLib.Notify("Redline","loaded  |  koji_xyz  (Page Up = menu)",5) end)
    log("[rl] v24 | MatchaUI | fps "..get_fps())
